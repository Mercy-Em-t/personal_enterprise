...
export default function AIInterviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setMessages([
        { role: 'system', content: `[SYSTEM] Blueprint Hydrated. Interactive Session initialized for Lead ID: ${leadId}` },
        ...
        <div className="p-4 bg-[#111] border-t border-gray-800 shrink-0">
          {interviewComplete ? (
            <div className="text-center text-sm text-green-500 p-3 border border-green-900/30 bg-green-900/10 rounded">
              Interview Complete. Please proceed to the final step.
              <a href={`/q/${leadId}`} className="block mt-4 text-white underline">Continue to Final Intake &rarr;</a>
            </div>
          ) : (
